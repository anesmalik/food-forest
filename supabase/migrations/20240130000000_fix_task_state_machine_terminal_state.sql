-- Refine task state-machine trigger: allow field mutations on non-terminal states,
-- but reject ALL updates on terminal states (completed, cancelled, missed are immutable).
